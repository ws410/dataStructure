...
public class StudentManagSystem extends SqList {
	...
	public StudentNode getStudentName(String name) throws Exception{
		for (int i = 0; i < length(); i++) { // 遍历整个顺序表
			StudentNode node = (StudentNode) super.get(i);// 调用父类的get方法
			if (node.getName().equals(name))
				return node;// 包含指定的姓名，返回该学生的信息
		}
		throw new Exception("姓名" + name + "不存在");// 拋出异常

	}
	public StudentNode modifu(int number) throws Exception{
		for (int i = 0; i < length(); i++) { // 遍历整个顺序表
			StudentNode node = (StudentNode) super.get(i);// 调用父类的get方法
			if (node.number== number){
				System.out.println("请输入要修改的成绩，按照大学英语和高等数学的顺序");
				Scanner sc=new Scanner(System.in);
	...
	public void displayNode(StudentNode node) {
		System.out.println("学号： " + node.number + " 姓名：" + node.name
				...
				+ " 高等数学成绩： " + node.math + "电话:" + node.phone + "地址:" +node.addr);
	}
	...
	public static void main(String[] args) throws Exception {
		...
		int n = sc.nextInt();
		System.out.println("请按学号、姓名、性别、大学英语、高等数学、电话和地址的顺序输入学生信息： ");
		StudentManagSystem L = new StudentManagSystem(maxSize, n);// 新建顺序表
		L.display();
		
		System.out.println("请输入需要查询学生的姓名或者学号： ");
		String str= sc.next();
		Pattern p=Pattern.compile("\\d+");
	    if(p.matcher(str).matches()){
	    	L.displayNode(L.get(Integer.parseInt(str))); // 取出成功，则输出该学生的信息
	    }else{
	    L.displayNode(L.getStudentName(str)); // 取出成功，则输出该学生的信息
	    }
