...
class CircleLinkQueue {
	...
	public void poll() throws Exception {
       // 移除带头结点的循环链队列中的队首元素并作为此函数的值返回该对象，如果此队列为空，则返回 null
		Node p = rear.next.next;// p指向待删除的队首结点
		if (p==rear)
            rear.next=rear;  //删除队首结点后，链队列变成了空链队列
        else
            
		     rear.next.next=p.next;// 删除队首结点
	}
// 打印函数，打印所有队列中的元素(队首到队尾)
	public void display() {
		if (rear.next!=rear) {
			for (Node p=rear.next.next;p!=rear.next;p=p.next)
				// 从队首到队尾
