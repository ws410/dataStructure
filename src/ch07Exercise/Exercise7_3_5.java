...
public class Exercise7_3_5 {
    ...
    boolean checkmax(BiTreeNode t)    //判断完全二叉树是否为大顶堆
    {
        ...
        if (p.lchild == null && p.rchild == null) {
            ...
            if (p.lchild != null && p.rchild != null) {
                if ((((RecordNode) p.lchild.data).key).compareTo(((RecordNode) p.data).key) <= 0 && (((RecordNode) p.rchild.data).key).compareTo(((RecordNode) p.data).key) <= 0) {                   
                    return checkmax(p.lchild) && checkmax(p.rchild);
                } else 
                {
                    return false;
                }
            } else if (p.lchild != null && p.rchild == null) {
                if ((((RecordNode) p.lchild.data).key).compareTo(((RecordNode) p.data).key) <= 0) {
                    return checkmax(p.lchild);
                } else 
                {
                    return false;
                }
                ...
                if ((((RecordNode) p.rchild.data).key).compareTo(((RecordNode) p.data).key) <= 0) {
                    return checkmax(p.rchild);
                } else 
                {
                    return false;
                }
